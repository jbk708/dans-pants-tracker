...
// Determine DB URL from TURSO_DATABASE_URL env var
function getDbUrl() {
    const url = process.env.TURSO_DATABASE_URL;
    if (!url) {
        throw new Error(
            'TURSO_DATABASE_URL env var is not set. ' +
            'Set it to your Turso database URL (e.g. libsql://tracker-jbk708.turso.io).'
        );
    }
    return url;
}
