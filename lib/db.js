...
const DB_URL    = process.env.TURSO_DATABASE_URL || process.env.LOCAL_DB_PATH;
const DB_TOKEN  = process.env.TURSO_AUTH_TOKEN;
// libsql:// URLs aren't supported by Node fetch — convert to https for the API call
const isLibsql  = DB_URL && DB_URL.startsWith('libsql://');
const apiHost   = isLibsql ? `https://${DB_URL.slice(8)}` : DB_URL;
const API_ROOT  = `${apiHost}/v2/pipeline`;
...
/**
 * Parse Turso HTTP API result rows (arrays) into objects using col names.
 * @param {{cols: string[], rows: any[]}} result
 */
function parseRows(result) {
    const colNames = (result.cols || []).map(c => c.name);
    return (result.rows || []).map(row =>
        Object.fromEntries(row.map((val, i) => [colNames[i], unwrap(val)]))
    );
}

/**
 * Unwrap Turso's typed values: {type: "text", value: "..."} → JS value.
 */
function unwrap(v) {
    ...
    switch (v.type) {
        ...
        case 'float':   return parseFloat(v.value);
...
async function sql(sql, args = []) {
    // Convert to Turso v2 typed format
    ...
    const result = await turso({ requests: [{ type: 'execute', stmt: { sql, args: typedArgs } }] });
    const r = result.results[0];
    if (r.error) throw new Error(r.error.message);
    return { rows: parseRows(r.response.result) };
}
...
async function sqlBatch(stmts) {
    const requests = stmts.map(s => {
        const typedArgs = (s.args || []).map(arg => {
            if (arg === null || arg === undefined) return { type: 'null', value: null };
            if (typeof arg === 'number' && Number.isInteger(arg)) return { type: 'integer', value: String(arg) };
            if (typeof arg === 'number') return { type: 'float', value: String(arg) };
            return { type: 'text', value: String(arg) };
        });
        return { type: 'execute', stmt: { sql: s.sql, args: typedArgs } };
    });
